...
frames = [frame_histogram, frame_bar_region, frame_bar_income]
...
def create_bar_chart_region():
    ...
    ax.set_xticks(range(len(population_2020['Region'].unique())))
...
def create_bar_chart_income():
    ...
    ax.set_ylabel('Total Population')
    ax.set_xticks(range(len(population_2020['IncomeGroup'].unique())))
    ax.set_xticklabels(ax.get_xticklabels(), rotation=45)
...
create_histogram()
create_bar_chart_region()
create_bar_chart_income()
...
Button(root, text="Back", command=previous_frame).grid(row=1, column=0, sticky='w')
Button(root, text="Next", command=next_frame).grid(row=1, column=1, sticky='e')
Button(root, text="Quit", command=root.quit).grid(row=1, column=2, sticky='e')

show_frame(frame_histogram)
